...
<body>
	<div id="wrapper">
		...
		<div id="contents">
			...
			<br>
			<br>
				<legend style="text-align: left;">자유게시판</legend>
				<hr>
				<br>
				<p id="textP">제목 : ${content.freeSubject }</p>
				<p id="textA">글번호 : ${content.freeNoticeNo } / 글쓴이 :
					${content.freeUserId } / 마지막 수정일 : ${content.freeUpDate } / 작성일 :
					${content.freeRegDate } / 조회수 : ${content.freeReadCount }</p>
				<textarea rows="10" cols="72" name="content">${content.freeContents }</textarea>
				<br>
				<div style="position: relative; left: 380px">
				<c:if test="${sessionScope.member.userId eq content.freeUserId}">
					<a href="/freeboard/modifyform?freeNoticeNo=${content.freeNoticeNo }">
					
					<input type="button" value="수정하기" id="buttonM"></a>
						
												
						<a href="/freeboard/delete?freeNoticeNo=${content.freeNoticeNo }" onclick="return deleteConfirm();"> 
						<input type="submit" value="삭제하기" id="buttonD"></a> 
						
						</c:if>
						<a href="/freeboard/list"><input type="button" value="목록으로 돌아가기" id="buttonL" onclick="javascript:history.back();"></a>
				</div>
